...
const SidebarGroupAction = React.forwardRef(({ className, asChild = false, showOnHover = false, ...props }, ref) => {
  const Comp = asChild ? Slot : "div";
  return <Comp ref={ref} data-sidebar="group-action" className={cn("absolute right-2 top-1 z-10 flex h-6 w-6 items-center justify-center rounded-md transition-opacity", showOnHover && "opacity-0 group-hover:opacity-100", className)} {...props} />;
});
